...
function extractGeminiResponse(data) {
  try {
    ...
    return { text: candidate.content.parts[0].text, finishReason: candidate.finishReason }
  } catch {
...
async function callGeminiEndpoint(url, extraHeaders, body) {
  ...
  const { text, finishReason } = extractGeminiResponse(data)
...
Use "sweating" if CO2 is above 5kg, otherwise "happy".`
  ...
  try {
    const text = await generateResponse(
      ...
      // M1 FIX: 600 tokens is ample for a 3-sentence JSON response
      // Previously 2048, which reserved ~4× more compute than needed.
...
${contextBlock}`
  ...
  try {
    const text = await generateResponse(systemPrompt, contents, {
      temperature: 0.7,
